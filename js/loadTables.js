...
//                tableData += '<td>' + qData.rp_id + '</td>';
            ...
            $('.select_vehicle').click(function () {
                select_repair_vehicle($(this).val(), function () {
                    var brid = $('#rp_branch').val();
                    var vid = $('#vid').val();
                    var mkdir = brid + '-'.concat(vid);
                    $('#vehicle_rep_Update_btn').removeClass('hidden');
                    $('#job_reg_reset_btn').removeClass('hidden');
//            $('#vehi_rep_save_btn').removeClass('hidden');
            ...
            $('.delete_vehicle').click(function () {
                var veID = $(this).val();
                confirm("Delete Branch", "<strong>Want to delete repair Data?</strong>", "Cancel", "Delete", function () {
                    delete_vehicles_repair(veID);
                });
            });
        }
    }, "json");
...
//                        + '<button class="btn btn-danger delete_fuel" value="' + qData.fuel_id + '"><i class="fa fa-trash-o fa-lg"></i>&nbsp;Delete</button></div></td>';
            ...
            totals += '<tr>';
            totals += '<td></td>';
            totals += '<td><strong>Total Quantity Of Fuel : ' + Number(qty).toFixed(2) + '</strong></td>';
            totals += '<td><strong>Total Cost : ' + Number(cost).toFixed(2) + '</strong></td>';
            totals += '<td></td>';
            totals += '</tr>';
            $('.fuel_report_Table tbody').html('').append(tableData);
...
function fuel_management_detailed_report(fromDate, toDate) {
    var br_id = '';
    $('.braComboDivfull').find('select').attr('selected', function () {
        br_id = $(this).val();
    });
//    var ve_id = '';
//    $('.vehicleComboBoxDiv').find('select').attr('selected', function() {
...
//    });
    ...
    $.post("views/loadTables.php", {table: "fuel_management_detail_report", fromDate: fromDate, toDate: toDate, br_id: br_id}, function (e) {
        if (e === undefined || e.length === 0 || e === null) {
            tableData += '<tr><th colspan="5" class="alert alert-warning text-center"> -- No Data Found -- </th></tr>';
            $('.fuel_full_report_Table tbody').html('').append(tableData);
//            $('.fuel_report_Table tfoot').html('');
        } else {
            $.each(e, function (index, qData) {
                index++;
